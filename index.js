...
function getAverageWordLength(array){
  ...
  let fifth = fourth.split(' ')
  let result = [(fifth.length/first ), fifth.length];
  return result
}
...
function getRandomFlavors(array1, array2, array3, array4){
  let bigArray = array1.concat(array2, array3, array4)
  let random = []
  for(let i=0; i<31; i++){
    random.push(bigArray[Math.floor(Math.random()*bigArray.length)])
  }
  return random
  /*code here*/
}
